...
func dateParser() {
    ...
    for (index, value) in dates.enumerated() {
        let text = Array(value.utf8)
        var date: DateTimeOffset? = nil

        #expect(DateUtils.tryParse(text, startIndex: 0, length: text.count, date: &date) == true, "Failed to parse date: \(value)")
        #expect(DateUtils.formatDate(date!) == expected[index])

        date = nil
        #expect(DateUtils.tryParse(text, startIndex: 0, date: &date) == true, "Failed to parse date: \(value)")
        #expect(DateUtils.formatDate(date!) == expected[index])

        date = nil
        #expect(DateUtils.tryParse(text, date: &date) == true, "Failed to parse date: \(value)")
        #expect(DateUtils.formatDate(date!) == expected[index])

        date = nil
        #expect(DateUtils.tryParse(value, date: &date) == true, "Failed to parse date: \(value)")
        #expect(DateUtils.formatDate(date!) == expected[index])
    }
}

@Test("Date parser invalid dates")
func dateParserInvalidDates() {
    let invalidDates = [
        "this is pure junk",
    ...
    for value in invalidDates {
        let text = Array(value.utf8)
        var date: DateTimeOffset? = nil
        #expect(DateUtils.tryParse(text, startIndex: 0, length: text.count, date: &date) == false)
        #expect(DateUtils.tryParse(text, startIndex: 0, date: &date) == false)
        #expect(DateUtils.tryParse(text, date: &date) == false)
        #expect(DateUtils.tryParse(value, date: &date) == false)
    }
}

@Test("Date parser months")
func dateParserMonths() {
...
@Test("Date parser timezones")
func dateParserTimeZones() {
    let cases: [(String, Int)] = [
        ("GMT", 0),
        ("UTC", 0),
        ("EDT", -400),
    ...
    for (value, offset) in cases {
        let text = "Wed, 1 Jan 2025 09:00:00 \(value)"
        var date: DateTimeOffset? = nil
        #expect(DateUtils.tryParse(text, date: &date) == true, "Failed to parse date: \(text)")
        #expect(date?.offsetMinutes == (offset / 100) * 60 + (offset % 100))
    }
}
